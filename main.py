...
class Pebble_bed:
    ...
    def process_data(self, calculate_dist=True, calculate_radial_dist=True, calculate_angles=True, calculate_center=True, radial_center=[0,0], verbose=True, level=0, log_delim='  '):
        log_print(f'Processing pbed data', verbose, level, log_delim)
        if calculate_center:
            self.center = np.array(radial_center+[self.data.z.mean()])
        if calculate_dist:
    ...
    def slice(self, dir_id=0, val='middle', *, tol=None, verbose=True, level=0, log_delim='  '):
        ...
        if isinstance(val, str) and val == 'middle':
            val = self.data[['x','y','z'][dir_id]].mean()
    ...
    def plot2D(self, field='id', dir_id=0, val='middle', colormap='turbo', xlim=None, ylim=None, tol=None, equal=True, field_title=None, new_fig=True, verbose=True, level=0, log_delim='  '):
        dir_id = str(dir_id)
        ...
        ax = plt.gca()
        ax.add_collection(p)
        plt.colorbar(p, label=field_title)
    ...
    def plot3D(self, field='id', colormap='turbo', view=None, xlim=None, ylim=None, zlim=None, sample_fraction=None, fast=False, force_slow=False, scatter_size=10, alpha=1, field_title=None, clim=None, new_fig=True, verbose=True, level=0, log_delim='  '):
        lim_fast = 1000
        ...
        if new_fig:
            plt.figure()
        ax = plt.gcf().add_subplot(111, projection="3d")  # , proj_type = 'ortho')
        ...
        if not fast:
            ...
            for _, row in data.iterrows():
                ...
                i_row += 1
            cb = plt.colorbar(cm.ScalarMappable(norm=norm, cmap=cmap), label=field_title, shrink=0.5, orientation="horizontal")
            ...
            cb = plt.colorbar(p, label=field_title, shrink=0.5, orientation="horizontal")
            cb.set_alpha(1)
            cb.draw_all()
    ...
    def show_Serpent_plot(self, plot_file_path, title=None, new_fig=True, xlim=None, ylim=None, verbose=True, level=0, log_delim='  '):
    ...
    def decompose_in_domains(self, n_domains_list, decomposition_types, filling_domain=None, center=None, shift_sectors=0, keep_subdomains=True, verbose=True, level=0, log_delim='  '):
        ...
        self.data['domain_id_0'] = self.data.domain_id
        ...
        for i_decomp in range(1, N_levels):
            ...
            domain_id = np.ones(len(self.data), dtype=int)*-1
            ...
            self.data[f'domain_id_{i_decomp}'] = domain_id
        ...
        for i_domain in range(self.data.domain_id.max()+1):
        ...
        if not keep_subdomains:
            self.data.drop([f'domain_id_{i_decomp}' for i_decomp in range(N_levels)], axis=1, inplace=True)
    ...
    def decompose_in_domains_simple(self, n_domains, decomposition_type, filling_domain=None, center=None, shift_sector=0, idle=True, verbose=True, level=0, log_delim='  '):
        ...
        if not isinstance(center, type(None)):
            self.center = center
            self.process_data(calculate_center=False)

        decomposition_type = str(decomposition_type).lower()
        target_npebbles_domains = np.floor(len(self.data) / n_domains).astype(int)
        ...
        if decomposition_type in ['0', 'random', 'n']:
            ...
            for i_domain in range(n_domains):
                for j in range(target_npebbles_domains):
            ...
            domain_id = np.ones((len(angles)), dtype=int)*-1
            i_pebble = 0
            for i_domain in range(n_domains):
                for j in range(target_npebbles_domains):
            ...
            domain_id = np.ones((len(radial_dist)), dtype=int)*-1
            i_pebble = 0
            for i_domain in range(n_domains):
                for j in range(target_npebbles_domains):
            ...
            domain_id = np.ones((len(axial_dist)), dtype=int)*-1
            i_pebble = 0
            for i_domain in range(n_domains):
                for j in range(target_npebbles_domains):
            ...
            domain_id = np.ones((len(dist)), dtype=int)*-1
            i_pebble = 0
            for i_domain in range(n_domains):
                for j in range(target_npebbles_domains):
